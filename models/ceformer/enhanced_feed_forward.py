...
from .layers import NonNegativeF, SinEmbedG, CosEmbedG, TopSample, DistributionSample
...
class EnhancedFeedForward(nn.Module):
    ...
    def __init__(self, embed_dim, hidden_dim, patch_shape,
                 dilation: tuple = (1, 2, 3), activation='gelu', dropout=0.1):
        ...
        self.conv_3x3_dilation = nn.ModuleList([
            nn.Conv2d(hidden_dim, hidden_dim // len(dilation), (3, 3), padding=d, dilation=d) for d in dilation])
        self.conv_1x1_dot_2 = nn.Conv2d(hidden_dim // len(dilation) * len(dilation), embed_dim, (1, 1))
    ...
    def seq2img(self, seq: torch.Tensor):
        ...
        img = seq.reshape(*seq.shape[:-2], *self.patch_shape, self.embed_dim)
        img = img.permute(0, 3, 1, 2)
        return img, class_token

    def img2seq(self, img: torch.Tensor, class_token: torch.Tensor):
        img = img.permute(0, 2, 3, 1)
        seq = img.reshape(*img.shape[:-3], -1, self.embed_dim)
